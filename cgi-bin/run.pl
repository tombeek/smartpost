...
foreach my $fileInDirectory (@$files) {
	...
	my $jobs = 	'';	# job description(s)
 	my $name = $tree->{ company } . '_' . $tree->{ job } . "_" . $tree->{ id } . '.html';	#name of individual job posting file
	...
	rename($name, '../htdocs/smartpost/live/' . $name) || print "Lack permission to rename " . $name . " or file does not exist.<br><br>";
	chmod(0755, '../htdocs/smartpost/live/' . $name) || print $!;
}

rename('../htdocs/smartpost/xml_files/' . $fileInDirectory, '../htdocs/smartpost/xml_files/processed/' . $fileInDirectory); #move processed xml file(s)
